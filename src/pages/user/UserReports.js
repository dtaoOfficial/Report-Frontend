...
const UserReports = () => {
  ...
  const [sortOrder, setSortOrder] = useState("latest"); // ✅ newest first
  ...
  // ✅ Auto-refresh when WebSocket message for "user" or "ALL" is received
  useEffect(() => {
    if (!lastUpdate?.data) return;
    const { type, data } = lastUpdate;
    const updateType = (type || "").toLowerCase();

    if (["user", "all", "manual"].includes(updateType)) {
      console.log("🔁 Live update detected in User Reports:", data.title);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Submit issues and track their live status in real-time.
            </p>
          </div>
                      ...
                      {reports.map((r) => (
                        ...
                        >
                          <div className="flex justify-between items-start gap-4">
                            ...
                            <div
                              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold uppercase tracking-wide ${getStatusColor(
                                r.status
                              )}`}
                            >
